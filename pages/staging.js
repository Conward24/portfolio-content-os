...
import { renderGraphic, getTemplatesForBrand } from '../lib/renderGraphic';
...
export default function Staging() {
  ...
  useEffect(() => {
    ...
    setCopy(draftCopy);
    // Extract hashtags from draft copy
    const extracted = extractHashtags(draftCopy);
    setHashtags(extracted);
  }, [idx]);

  // Sync hashtags back into copy whenever they change
  useEffect(() => {
    if (!copy) return;
  ...
  useEffect(() => {
    if (!canvasRef.current || !meta.brand) return;
    const canvas = canvasRef.current;
    const lines = copy.split('\n').filter(Boolean);
    const data = {
      stat: lines.find(l => /\d/.test(l) && l.length < 10) || '64%',
      statLabel: 'health risk assessment completion',
      headline: lines[0] || 'Headline',
      body: lines.slice(1, 4).join(' ').slice(0, 200),
      quote: lines.find(l => l.length > 30 && l.length < 120) || lines[0] || 'Your quote here',
      sentimentLabel: meta.sentiment ? SENTIMENT_TYPES[meta.sentiment]?.name : 'Emerging signal',
      flaggedDay: 'Tuesday', publishedDay: 'Thursday', publication: 'TechCrunch',
      topic: lines[0] || 'Market signal', daysAhead: '11 days',
    };
    renderGraphic(canvas, { brand: meta.brand, template, platform, data, images: {} });
  }, [copy, template, platform, meta, idx]);
    ...
    <Layout title="Staging" active="staging">
      ...
      <div className="page-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, maxWidth: 1000 }}>
        ...
        <div>
          ...
          <div style={{ background: '#f0f0f0', borderRadius: 12, padding: 16, marginBottom: 12 }}>
