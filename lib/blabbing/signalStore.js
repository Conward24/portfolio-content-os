...
// Resolve Redis creds across the naming conventions this project has accumulated.
// The live store (cerise-ocean / grown-monitor host) was attached under a
// `NewRedis_` prefix because the old, dead `KV_REST_API_*` vars still occupied
// the plain names — so the prefixed vars must take priority.
const redis = new Redis({
  url: process.env.NewRedis_KV_REST_API_URL || process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL,
  token: process.env.NewRedis_KV_REST_API_TOKEN || process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN,
});
