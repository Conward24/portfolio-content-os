...
// ── fallback: labeled "Sentiment:" / "Emotions: joy 80, trust 72, ..." footer ──
function extractLabeledMetrics(text) {
  const out = {};
  const sent = text.match(/Sentiment:\s*(Positive|Neutral|Negative)/i);
  if (sent) out.sentiment = sent[1][0].toUpperCase() + sent[1].slice(1).toLowerCase();

  const emoLine = text.match(/Emotions?:\s*([^\n]+)/i);
  if (emoLine) {
    const emotions = {};
    for (const [, k, v] of emoLine[1].matchAll(/([a-z]+)\s*[:=]?\s*(\d{1,3})/gi)) {
      const key = k.toLowerCase();
      if (EMOTION_KEYS.includes(key)) emotions[key] = Math.min(100, parseInt(v, 10));
    }
    if (Object.keys(emotions).length) out.emotions = emotions;
  }
...
// ─────────────────────────────────────────────────────────────────
export function deriveContentSignal({ emotions, sentiment }) {
  if (emotions) {
    const e = emotions;
