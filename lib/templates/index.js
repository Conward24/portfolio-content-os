...
function henwayStat(t, w, h2, data) {
  const k = h2 / 1080, S = (v) => Math.round(v * k); // scale to canvas height; k=1 for square (no regression)
  ...
  const kicker = data.kicker || 'By the numbers';
  return col(
    { width: w, height: h2, backgroundColor: t.bgDark, fontFamily: t.font, padding: S(84), justifyContent: 'space-between' },
    [
      row({ key: 'top', alignItems: 'center', justifyContent: 'space-between' }, [
        logoOrWordmark(data, { color: t.accent, font: t.font, size: S(36) }),
        text({ fontSize: S(15), fontWeight: 700, letterSpacing: 3, color: t.accent, textTransform: 'uppercase' }, kicker),
      ]),
      col({ key: 'mid', flexGrow: 1, justifyContent: 'center' }, [
        text({ fontSize: S(260), fontWeight: 800, color: t.accent, lineHeight: 0.92, letterSpacing: -8 }, stat),
        text({ fontSize: S(38), fontWeight: 600, color: t.onDark, marginTop: S(16), maxWidth: w * 0.78, lineHeight: 1.2, letterSpacing: -0.5 }, statLabel),
      ]),
      col({ key: 'foot' }, [
        h('div', { style: { width: S(96), height: S(6), backgroundColor: t.accent, marginBottom: S(22) } }),
        text({ fontSize: S(30), fontWeight: 700, color: t.onDark, letterSpacing: -0.5 }, headline),
        text({ fontSize: S(18), color: t.onDarkSoft, marginTop: S(6) }, t.website),
      ]),
    ]
  );
}
...
function henwayQuote(t, w, h2, data) {
  ...
  return row(
    ...
    [
      h('div', { key: 'bar', style: { width: S(24), height: h2, backgroundColor: t.accent } }),
      col({ key: 'body', flexGrow: 1, padding: S(88), justifyContent: 'space-between' }, [
        logoOrWordmark(data, { color: t.ink, font: t.font, size: S(34) }),
        text({ fontSize: S(54), fontWeight: 700, color: t.ink, lineHeight: 1.25, letterSpacing: -1 }, quote),
        col({}, [
          h('div', { style: { width: S(70), height: S(4), backgroundColor: t.accent, marginBottom: S(18) } }),
          text({ fontSize: S(26), fontWeight: 600, color: t.ink }, attribution),
        ]),
      ]),
    ]
  );
...
function henwaySignal(t, w, h2, data) {
  ...
  const kicker = data.kicker || 'The thesis';
  return col(
    { width: w, height: h2, backgroundColor: t.bgDark, fontFamily: t.font, padding: S(84), justifyContent: 'space-between' },
    [
      row({ key: 'top', alignItems: 'center' }, [
        h('div', { style: { width: S(14), height: S(14), backgroundColor: t.accent, marginRight: S(14) } }),
        text({ fontSize: S(16), fontWeight: 700, letterSpacing: 3, color: t.accent, textTransform: 'uppercase' }, kicker),
      ]),
      col({ key: 'mid', flexGrow: 1, justifyContent: 'center' }, [
        text({ fontSize: S(64), fontWeight: 800, color: t.onDark, lineHeight: 1.12, letterSpacing: -1.5, maxWidth: w * 0.86 }, headline),
        text({ fontSize: S(30), fontWeight: 400, color: t.onDarkSoft, marginTop: S(32), lineHeight: 1.4, maxWidth: w * 0.82 }, body),
      ]),
      row({ key: 'foot', alignItems: 'center', justifyContent: 'space-between' }, [
        logoOrWordmark(data, { color: t.accent, font: t.font, size: S(32) }),
        text({ fontSize: S(18), color: t.onDarkSoft }, t.website),
      ]),
    ]
  );
}

// One flexible carousel slide (4:5). Drive per-slide via data:
// { kicker, headline, body, cta, progress, swipe, dark }
function henwayCarousel(t, w, h2, data) {
  const k = h2 / 1350, S = (v) => Math.round(v * k);
  const dark = !!data.dark;
  const bg = dark ? t.bgDark : t.bg;
  const fg = dark ? t.onDark : t.ink;
  const fgSoft = dark ? t.onDarkSoft : '#5A5A5A';
  ...
  return col(
    { width: w, height: h2, backgroundColor: bg, fontFamily: t.font, padding: S(96), justifyContent: 'space-between' },
    [
      row({ key: 'top', alignItems: 'center', justifyContent: 'space-between' }, [
        kicker
          ? row({ alignItems: 'center' }, [
              h('div', { style: { width: S(16), height: S(16), backgroundColor: t.accent, marginRight: S(14) } }),
              text({ fontSize: S(22), fontWeight: 800, letterSpacing: 3, color: t.accent, textTransform: 'uppercase' }, kicker),
            ])
          : col({}, []),
        progress ? text({ fontSize: S(22), fontWeight: 700, color: fgSoft }, progress) : col({}, []),
      ]),
      col({ key: 'mid', flexGrow: 1, justifyContent: 'center' }, [
        text({ fontSize: S(80), fontWeight: 800, color: fg, lineHeight: 1.08, letterSpacing: -2, maxWidth: w * 0.92 }, headline),
        body ? text({ fontSize: S(37), fontWeight: 400, color: fgSoft, marginTop: S(34), lineHeight: 1.35, maxWidth: w * 0.9 }, body) : col({}, []),
        cta
          ? col({ marginTop: S(40) }, [
              h('div', { style: { width: S(90), height: S(6), backgroundColor: t.accent, marginBottom: S(22) } }),
              text({ fontSize: S(40), fontWeight: 800, color: t.accent, lineHeight: 1.2, maxWidth: w * 0.9 }, cta),
            ])
          : col({}, []),
      ]),
      row({ key: 'foot', alignItems: 'center', justifyContent: 'space-between' }, [
        logoOrWordmark(data, { color: dark ? t.accent : t.ink, font: t.font, size: S(34) }),
        swipe ? text({ fontSize: S(28), fontWeight: 700, color: t.accent }, swipe) : text({ fontSize: S(22), color: fgSoft }, t.website),
      ]),
    ]
  );
}

// Chick-forward card (white bg). Real egg lockup + a varied chick cutout
// (data.mascot -> mascotDataUri). Optional eyebrow pill / subline / attribution.
function henwayChick(t, w, h2, data) {
  const k = h2 / 1920, S = (v) => Math.round(v * k);
  const ASPECT = { chick: 247 / 365, peeking: 404 / 560, pointing: 428 / 560, thumbsup: 384 / 560, translating: 549 / 560 };
  const headline = data.headline || '';
  const eyebrow = data.eyebrow || '';
  const subline = data.subline || '';
  const attribution = data.attribution || '';
  const hlSize = data.hlSize || 112;
  const side = data.chickSide === 'left' ? 'left' : 'right';
  const asp = ASPECT[data.mascot] || 0.7;
  const chH = S(data.chickH || 880);
  const chW = Math.round(chH * asp);
  const children = [
    logoOrWordmark(data, { color: t.ink, font: t.font, size: S(48) }),
    eyebrow
      ? row({ marginTop: S(70) }, [
          text({ backgroundColor: t.accent, color: '#2A2A2A', fontSize: S(30), fontWeight: 800, letterSpacing: 3, paddingTop: S(12), paddingBottom: S(12), paddingLeft: S(28), paddingRight: S(28), borderRadius: 999, textTransform: 'uppercase' }, eyebrow),
        ])
      : null,
    text({ marginTop: S(eyebrow ? 26 : 90), fontSize: S(hlSize), fontWeight: 800, color: t.ink, lineHeight: 1.05, letterSpacing: -2, maxWidth: w * 0.9 }, headline),
    h('div', { style: { marginTop: S(38), width: S(160), height: S(16), backgroundColor: t.accent, borderRadius: S(8) } }),
    subline ? text({ marginTop: S(34), fontSize: S(46), fontWeight: 600, color: t.charcoal, maxWidth: w * 0.66, lineHeight: 1.22 }, subline) : null,
    attribution ? text({ marginTop: S(34), fontSize: S(38), fontWeight: 700, color: t.inkSoft, maxWidth: w * 0.58 }, attribution) : null,
    data.mascotDataUri
      ? h('img', { src: data.mascotDataUri, width: chW, height: chH, style: { position: 'absolute', bottom: 0, [side]: S(40), objectFit: 'contain' } })
      : null,
    text({ position: 'absolute', bottom: S(58), left: S(90), fontSize: S(30), fontWeight: 600, color: '#9a9a86' }, t.website),
  ].filter(Boolean);
  return col(
    { width: w, height: h2, backgroundColor: t.bg, fontFamily: t.font, padding: S(90), position: 'relative', justifyContent: 'flex-start' },
    children
  );
}
...
const TEMPLATES = {
  ...
  henway:   { stat: henwayStat, quote: henwayQuote, signal: henwaySignal, carousel: henwayCarousel, chick: henwayChick },
...
const TEMPLATE_META = {
  ...
  henway: [
    ...
    { id: 'chick', label: 'Chick card', desc: 'White · chick + headline' },
  ],
  blabbing: [
