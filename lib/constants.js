...
/**
 * Channel labels a post can carry (post.channelLabel). The badge on the calendar
 * grid and /today is coloured by BRAND for the four social labels; EMAIL is the
 * exception and gets its own colour so a Kit send reads as "not a social post"
 * at a glance regardless of brand.
 */
export const CHANNEL_LABELS = {
  PERSONAL: { id: 'PERSONAL', name: 'LinkedIn personal' },
  COMPANY: { id: 'COMPANY', name: 'LinkedIn company' },
  TIKTOK: { id: 'TIKTOK', name: 'TikTok' },
  CROSSOVER: { id: 'CROSSOVER', name: 'Crossover' },
  EMAIL: { id: 'EMAIL', name: 'Email (Kit)', bg: '#1F6FEB', fg: '#fff', light: '#E8F0FE', border: '#B6CCF5', text: '#174EA6' },
};

/** Badge colours for a label that overrides the brand colour (only EMAIL today). */
export function labelColors(channelLabel) {
  const l = CHANNEL_LABELS[channelLabel];
  return l && l.bg ? l : null;
}
